...
try:
    ...
    from data.kullanici_verileri import (
        ...
        hatirlatma_sil,
        konu_alarmi_kur,
        konu_alarmlarini_getir,
        konu_alarmi_sil,
        konu_alarmi_gorulenleri_guncelle,
    )
        ...
        try:
            ...
            if kendi_kayitlarim:
                for i, meta in reversed(kendi_kayitlarim):
                    ...
                    if st.button(f"🕘 {goruntu_metni}", key=f"gecmis_{i}", width="stretch"):
...
if mod == "Genel Haber Modu":
    ...
    with col_button:
        analiz_et = st.button("🔎 Analiz Et", width="stretch")

    # "Yenile" butonu (sonuç bölümünde) cache'i atlayıp aynı konuyu zorla
    # yeniden sorgulamak için bu bayrağı set eder.
    zorla_yenile = st.session_state.pop("zorla_yenile_tetikle", False)
    if zorla_yenile:
        konu = st.session_state.get("son_konu", konu)
        analiz_et = True
    ...
    if analiz_et:
        if not AJANLAR_YUKLENDI:
            ...
            try:
                cache_kaydi = None if zorla_yenile else cache_getir(konu)

                if cache_kaydi:
                    haberler = cache_kaydi["haberler"]
                    analiz = cache_kaydi["analiz"]
                    genis_arama_yapildi = cache_kaydi["genis_arama_yapildi"]
                    cache_zamani = cache_kaydi["kaydedilme_zamani"]
                else:
                    ...
                    if haberler:
                        ...
                        cache_kaydet(
                            ...
                            haberler=haberler,
                            analiz=analiz,
                            genis_arama_yapildi=genis_arama_yapildi,
                        )
                        cache_zamani = time.time()
    ...
    if st.session_state.get("son_analiz"):
        ...
        cache_zamani = st.session_state.get("son_cache_zamani")
        if cache_zamani:
            dakika = max(0, int((time.time() - cache_zamani) / 60))
            col_cache_not, col_cache_buton = st.columns([5, 1])
            with col_cache_not:
                st.caption(f"🕘 Sonuçlar {dakika} dakika önce güncellendi.")
            with col_cache_buton:
                if st.button("🔄 Yenile", key="cache_yenile_btn", width="stretch"):
                    cache_temizle(son_konu)
                    st.session_state["zorla_yenile_tetikle"] = True
                    st.rerun()
        ...
        with col_alarm:
            if kullanici:
                if st.button("🔔 Bu Konuda Haber Alarmı Kur"):
                    try:
                        konu_alarmi_kur(kullanici["id"], son_konu)
                        st.success(f"'{son_konu}' için haber alarmı kuruldu. Kontrolü Favorilerim sayfasından yapabilirsiniz.")
                    except Exception:
                        st.error("😕 Alarm kurulurken bir sorun oluştu.")
        with col_pdf:
            try:
                pdf_verisi = genel_analiz_pdf_uret(
                    son_konu, analiz.get("tldr", ""), analiz.get("bias_analysis", {}), haberler
                )
                st.download_button(
                    "📄 PDF İndir", data=pdf_verisi,
...
# =============================================================================
    ...
    if AJANLAR_YUKLENDI:
        ...
        finans_zorla_yenile = st.session_state.pop("finans_zorla_yenile_tetikle", False)
        if finans_zorla_yenile:
            finansal_analiz_et = True
        ...
        if finansal_analiz_et:
            if not ticker:
                ...
                try:
                    ...
                    # Haber toplama (Tavily) Katman 3 cache'inden karşılanır; fiyata bağlı
                    # olduğu için finansal_analiz_yap (Claude) her seferinde güncel
                    # fiyatla yeniden çalıştırılır, cache'lenmez.
                    finans_cache_kaydi = None if finans_zorla_yenile else cache_getir(finansal_sorgu)
                    if finans_cache_kaydi:
                        finansal_haberler = finans_cache_kaydi["haberler"]
                        finans_haber_cache_zamani = finans_cache_kaydi["kaydedilme_zamani"]
                    else:
                        with st.spinner(f"'{secilen_varlik_adi}' hakkında haberler toplanıyor..."):
                            finansal_haberler = haberleri_getir(finansal_sorgu)
                        cache_kaydet(finansal_sorgu, haberler=finansal_haberler)
                        finans_haber_cache_zamani = time.time()
                    ...
                    st.session_state["finans_sorgu"] = finansal_sorgu
                    ...
                    st.session_state["finans_haber_cache_zamani"] = finans_haber_cache_zamani
    ...
    if AJANLAR_YUKLENDI and st.session_state.get("finans_analiz"):
        ...
        if finansal_haberler:
            haber_cache_zamani = st.session_state.get("finans_haber_cache_zamani")
            if haber_cache_zamani:
                haber_dakika = max(0, int((time.time() - haber_cache_zamani) / 60))
                col_finans_cache_not, col_finans_cache_buton = st.columns([5, 1])
                with col_finans_cache_not:
                    st.caption(f"🕘 Haberler {haber_dakika} dakika önce güncellendi.")
                with col_finans_cache_buton:
                    if st.button("🔄 Yenile", key="finans_cache_yenile_btn", width="stretch"):
                        cache_temizle(st.session_state.get("finans_sorgu", ""))
                        st.session_state["finans_zorla_yenile_tetikle"] = True
                        st.rerun()
...
# =============================================================================
    ...
    if not AJANLAR_YUKLENDI:
        ...
        st.markdown("### 🔔 Konu/Haber Alarmlarım")
        st.caption(
            "Takip ettiğiniz bir konuda yeni haber çıktığında burada bildirim görürsünüz "
            "(bildirimler yalnızca uygulama içinde gösterilir, dışarıya push bildirimi gönderilmez)."
        )
        try:
            konu_alarmlari = konu_alarmlarini_getir(kullanici["id"])
        except Exception:
            konu_alarmlari = []
            st.error("😕 Konu alarmları yüklenirken bir sorun oluştu.")

        if not konu_alarmlari:
            st.caption(
                "Henüz takip ettiğiniz bir konu yok. Genel Haber Modu'nda bir analiz yapıp "
                "'Bu Konuda Haber Alarmı Kur' butonuyla ekleyebilirsiniz."
            )
        else:
            if st.button("🔄 Yeni Haberleri Kontrol Et", key="konu_alarm_kontrol_buton"):
                if not AJANLAR_YUKLENDI:
                    st.error("Ajan modülleri yüklenemediği için kontrol yapılamıyor.")
                else:
                    simdi = datetime.now(timezone.utc)
                    with st.spinner("Takip edilen konularda yeni haber var mı kontrol ediliyor..."):
                        for alarm in konu_alarmlari:
                            if alarm["siklik"] == "kapali":
                                continue
                            if alarm["siklik"] == "gunluk" and alarm["son_kontrol_tarihi"]:
                                son_kontrol = datetime.fromisoformat(alarm["son_kontrol_tarihi"])
                                if (simdi - son_kontrol).total_seconds() < 24 * 3600:
                                    continue

                            try:
                                guncel_haberler = haberleri_getir(alarm["konu"])
                            except Exception:
                                st.warning(f"'{alarm['konu']}' için haberler kontrol edilemedi.")
                                continue

                            guncel_urller = [h.get("URL", "") for h in guncel_haberler if h.get("URL")]
                            daha_once_gorulen = set(json.loads(alarm["gorulen_haber_urlleri"]))

                            if alarm["son_kontrol_tarihi"] is None:
                                # İlk kontrol: mevcut haberlerin tamamı zaten "eski" kabul
                                # edilir, ilk seferde her şeyi "yeni" diye bildirmeyiz.
                                pass
                            else:
                                yeni_haberler = [
                                    h for h in guncel_haberler
                                    if h.get("URL") and h["URL"] not in daha_once_gorulen
                                ]
                                if yeni_haberler:
                                    st.success(
            ...
            for alarm in konu_alarmlari:
                siklik_etiketi = {"anlik": "Her kontrolde", "gunluk": "Günde bir", "kapali": "Kapalı"}.get(
                    alarm["siklik"], alarm["siklik"]
                )
                col_ad, col_sil = st.columns([5, 1])
                col_ad.write(f"🔖 **{alarm['konu']}** · Bildirim sıklığı: {siklik_etiketi}")
                if col_sil.button("Sil", key=f"konu_alarm_sil_{alarm['id']}"):
                    konu_alarmi_sil(alarm["id"], kullanici["id"])
                    st.rerun()
...
# =============================================================================
    ...
    if not AJANLAR_YUKLENDI:
        ...
        if not portfoy_kayitlari:
            ...
            try:
                csv_verisi = portfoy_csv_uret(portfoy_kayitlari)
                st.download_button(
                    "⬇️ Portföyü CSV Olarak İndir", data=csv_verisi,
                    file_name="portfoyum.csv", mime="text/csv",
                )
            except Exception:
